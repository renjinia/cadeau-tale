...
const View = ({addResponseFromHome,setDeleteResponseFromView,deleteResponseFromWishlist}) => {
  ...
  return (
    <>
    <Row droppable="true" onDragOver={dragOverView} onDrop={e=>wishlistProductDragOverView(e)} >
      {
        allproducts?.length>0?
        allproducts.map(product=>(
          <Col key={product?.id} className='mb-3' sm={12} md={6} lg={4}>
          <ImageCard  setDeleteProductFromImageCard={setDeleteProductFromImageCard} displayData={product} />
          </Col>
        ))
        :
        <div className='fw-bolder text-danger fs-5'>No Product Details</div>
      }
    </Row>
    </>
  )
